...
## 真机设置(全局触发)
灵动岛卡片必须由快捷指令触发。iOS 不允许后台 App 自行打开麦克风,因此录音由系统的「听写文本」完成:
1. 打开「快捷指令」→ 新建。
...
## 工程结构
- `BinanceVoiceAgent/` App target:`AppState` 状态机、`Services/`(Speech / Parser / Biometric / Notification / Order / IslandFlow)、`Intents/`、`Views/`。
...
xcodegen generate
open BinanceVoiceAgent.xcodeproj   # 选真机运行(语音识别与灵动岛在真机上最完整)
```
